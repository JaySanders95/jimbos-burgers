...
class BookingCreateView(LoginRequiredMixin, CreateView):
    ...
    def find_free_table(self, capacity, date, time):
        ...
        available_tables = Table.objects.filter(party_size=capacity, is_available=True)

        for table in available_tables:
            #check availability for specified date and time
            existing_booking = Booking.objects.filter(table=table, booking_date=booking_date, booking_time=booking_time).first()
