...
from bookings.views import BookingCreateView, BookingListView, BookingUpdateView
...
self.table = Table.objects.create(table_number=1, capacity=4, is_available=True)self.table = Table.objects.create(table_number=1, capacity=4, is_available=True)
    def test_booking_edit_view(self):
        ...
        response = view(request, pk=booking.pk)
