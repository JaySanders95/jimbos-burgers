...
class BookingForm(forms.ModelForm):
    ...
    def clean_notes(self):
        # Checks if any special characters
        if any (char in notes for chat in "!£$%^&*()")
    ...
    def save(self, commit=True):
        ...
        booking.table = self.find_best_table(
            booking.num_guests, booking.date, booking.time)
    ...
    def find_best_table(self, requested_capacity, requested_date, requested_time):
        ...
        available_tables = Table.objects.filter(
            ...
            Q(capacity__gte=requested_capacity + extra_capacity,
              capacity__in=[2, 3, 4, 5, 6, 10, 12])
