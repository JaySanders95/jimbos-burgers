...
class BookingCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        try:
            ...
            time_choice_value = form.cleaned_data['time']  # Integer representing time choice
            time_choice = dict(TIME_CHOICES).get(time_choice_value)  # Get the time choice

            # Ensure the time_choice is valid
            if time_choice is None:
                raise ValueError("Invalid time choice.")

            # Convert time_choice to a datetime.time object
            time_choice = datetime.strptime(time_choice, "%H:%M").time()

            # Combine date and time_choice to create a datetime object
            booking_datetime = datetime.combine(date, time_choice)
...
class BookingUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        try:
            ...
            #if date < timezone.now().date():
                #raise ValueError("You can't book tables in the past.")
            booking_datetime = datetime.combine(date, time(hour=time_choices[booking.time][0]))

            # Check if the booking_datetime is in the past
            if booking_datetime < timezone.now():
                raise ValueError("You can't update bookings to a past date.")
