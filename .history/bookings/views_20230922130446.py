...
class BookingListView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        suitable_table = list(Table.objects.filter)

        if names:
            return Booking.objects.filter(names__icontains=names)
        if query:
        ...
        if self.request.user.is_staff:
            ...
            return Booking.objects.filter(date__gte=dates.date.today())
        else:
            # This customer's bookings today or later
            return Booking.objects.filter(
                customer=self.request.user, date__gte=dates.date.today()
            )
